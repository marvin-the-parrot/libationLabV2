 ...
 * however for the sake of simplicity it is tested against the message endpoint.
 */
...
public class SecurityTest implements TestData {
    ...
    private Message message = Message.MessageBuilder.message()
     ...
     * Feel free to remove / disable / adapt if you do not use Method Security
     * (e.g. if you prefer Web Security to define who may perform which actions)
     * or want to use Method Security on the service layer.
     */
    @Test
    public void ensureSecurityAnnotationPresentForEveryEndpoint() {
        ...
        assertThat(notSecured.size())
            .as("Most rest methods should be secured. If one is really intended for public use, "
                + "explicitly state that with @PermitAll. "
                + "The following are missing: \n" + notSecured.stream().map(pair -> "Class: "
                + pair.getLeft() + " Method: " + pair.getRight()).reduce("", (a, b)
                -> a + "\n" + b))
            .isZero();
